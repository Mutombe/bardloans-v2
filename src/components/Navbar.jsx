  ...
  return (
    <>
      <motion.nav
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-white/95 backdrop-blur-xl shadow-lg shadow-navy/5' : 'bg-transparent'}`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 lg:h-20">
            <motion.a href="#home" className="flex-shrink-0" whileHover={{ scale: 1.02 }}>
              <img src="/logo.png" alt="Bard Loans" className={`h-8 lg:h-10 w-auto transition-all duration-500 ${scrolled ? '' : 'brightness-0 invert'}`} loading="eager" />
            </motion.a>
            <div className="hidden lg:flex items-center gap-7">
              {navLinks.filter(l => l.name !== 'Loan Calculator').map((link) => (
                <a key={link.name} href={link.href} className={`text-sm font-semibold transition-colors duration-300 uppercase tracking-wide ${scrolled ? 'text-navy/70 hover:text-primary' : 'text-white/70 hover:text-white'}`}>
                  {link.name}
                </a>
              ))}
              <a href="#calculator" className={`px-6 py-2.5 rounded-full text-sm font-extrabold uppercase tracking-wide transition-all duration-500 ${scrolled ? 'bg-primary text-white shadow-lg hover:bg-primary-dark' : 'bg-white text-navy shadow-lg hover:bg-white/90'}`}>
                Apply Now
              </a>
            </div>
            <button onClick={() => setMobileOpen(true)} className={`lg:hidden p-2 cursor-pointer ${scrolled ? 'text-navy' : 'text-white'}`}>
